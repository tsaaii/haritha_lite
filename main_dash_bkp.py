...
def route_and_authenticate(pathname, search):
    ...
    # ✅ ADD PUBLIC ROUTES HERE (no auth required)
    if pathname == '/Kadapa/Rayachoti':
        return 'kadapa_rayachoti', False, {}, ''  # Always public, no auth needed
...
# ADD THE KADAPA ROUTE HERE - MODIFY THE FUNCTION ABOVE!
...
def render_layout(theme_name, is_authenticated, current_page, user_data, error_message):
    ...
    try:
        if current_page == 'login':
            ...
            return layout
        elif current_page == 'kadapa_rayachoti':  # ✅ ADD THIS CASE
...
@callback(
    ...
    [Input('demo-login-btn', 'n_clicks'),
     ...
     Input('username-password-login-btn', 'n_clicks'),  # ✅ ADD THIS
     Input('back-to-public-btn', 'n_clicks')],
    [State('access-pin', 'value'),
     State('manual-email', 'value'),
     State('username-input', 'value'),          # ✅ ADD THIS
     State('password-input', 'value'),          # ✅ ADD THIS
     State('current-page', 'data')],
    prevent_initial_call=True
)
def handle_login_actions(demo_clicks, admin_clicks, dev_clicks, 
                        viewer_clicks, pin_clicks, manual_clicks, 
                        username_password_clicks, back_clicks,    # ✅ ADD THIS
                        access_pin, manual_email, username, password,  # ✅ ADD THESE
                        current_page):
    """Handle all login actions including username/password"""
    ...
    # ✅ NEW: Username/Password Authentication
        ...
        if not username or not password:
            print("DEBUG: Username/password missing")
        ...
        if validate_user_credentials(username, password):
            ...
            print(f"DEBUG: Username/password login successful for {username}")
            return '/dashboard'
        else:
            print(f"DEBUG: Invalid credentials for {username}")
            return '/login?error=invalid_credentials'
    
    # Demo login methods (keep existing)
